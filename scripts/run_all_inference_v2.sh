#!/bin/bash
# Run inference for all datasets × 2 v2 checkpoints × {transcribe, continue}
# 6 datasets × 2 checkpoints × 2 tasks = 24 runs

set -euo pipefail

MODEL_ROOT="/capstor/store/cscs/swissai/infra01/MLLM/ablations/audio"
CHECKPOINTS=(
    "audio-weight-0-phase-transition-v2"
    "audio-weight-1-phase-transition-v2"
)
TASKS=("transcribe" "continue")

WAV_DIR="results/inference"
OUTPUT_DIR="results/inference_greedy_v2"
